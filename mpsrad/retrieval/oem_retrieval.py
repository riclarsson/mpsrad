...
class oem_retrieval():
    ...
    def load_data(self,data_type='radiometer',filename='',signal=[],noise=[],freq=[]):
        """
        AUTHOR:
          Hayden Smotherman
        DESCRIPTION:
        ...
        # Load data from a .raw file from the 210MHz radiometer
            ...
            self.signal = np.array(Calibrated._signal)
            self.noise  = np.array(Calibrated._noise)
            ...
            if filename[0] == 'a':
                ...
                self.signal = Calibrated._signal
                self.noise  = Calibrated._noise
    ...
    def mean_retrieval(self,use_data=0,filtered=False,plot_results=False,plot_statistics=False):
        """
        AUTHOR:
          Hayden Smotherman
        DESCRIPTION:
        ...
        if len(self.signal.shape) > 1:
            self.average_signal = np.mean(self.fit_signal[use_data:],axis=0)
            self.average_noise  = np.mean(self.fit_noise[use_data:], axis=0)
        else:
