...
class oemWidget:
	...
	def refresh_oem(self, i):
		...
		self.Retrieval.load_data(data_type='numpy',signal=[signal],noise=[noise],freq=freq)
		self.Retrieval.process_data(lims=self.window)
		...
		if self.physical_sigma[i]:
			...
			self.Retrieval.mean_retrieval(filtered=DoFilter,shift_freq=DoShift,sigma=sigma)
